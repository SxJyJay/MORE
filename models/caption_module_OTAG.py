...
class OTAGCaptionModule(nn.Module):
    def __init__(self, vocabulary, embeddings, emb_size=300, feat_size=128, hidden_size=512, num_proposals=256,
        ...
        self.rel_feat = MultiHead(feat_size, feat_size, num_heads=4,
                dropout=0.1, post='111')

        # top-down attention module
        self.map_fst_order_node_feat = nn.Linear(feat_size*3, feat_size)
        self.map_scd_order_node_feat = nn.Linear(feat_size*3, feat_size)
        ...
        # self.dropin = nn.Dropout(args.dropin)
        self.drop1 = nn.Dropout(0.5)
        self.drop2 = nn.Dropout(0.5)

    def _step(self, step_input, target_feat, tri_feats, hidden_1, hidden_2, object_masks, prev_mask):
        '''
            recurrent step
        ...
        hidden_1 = self.drop1(hidden_1)

        tri_feats = self.rel_feat(tri_feats)

        # top-down attention
        combined = self.map_feat(tri_feats)
        combined += self.map_hidd(hidden_1).unsqueeze(1) # batch_size, num_locals, hidden_size
        combined = torch.tanh(combined)
        scores = self.attend(combined) # batch_size, num_locals, 1
        #scores.masked_fill_(object_masks == 0, float('-1e30'))
        # ipdb.set_trace()

        masks = F.softmax(scores, dim=1) # batch_size, num_proposals, 1
        attended = tri_feats * masks
    ...
    def _get_relation_feat(self, data_dict, obj_feats, target_ids):
        rel_feats = data_dict["edge_feature"] # batch_size, num_proposals, num_locals, feat_size
        batch_size = rel_feats.shape[0]
        ...
        adjacent_mat = data_dict["adjacent_mat"] # batch_size, num_proposals, num_proposals
        nei_indices = torch.gather(adjacent_mat, 1,
            target_ids.view(batch_size, 1, 1).repeat(1, 1, self.num_proposals)).squeeze(1) # batch_size, num_proposals

        # ipdb.set_trace()
        nei_feats = []
        for i in range(batch_size):
            of_i = obj_feats[i, nei_indices[i].nonzero()[:, 0]]
            nei_feats.append(of_i)
        nei_feats = torch.stack(nei_feats, 0)

        return rel_feats, nei_feats

    def _get_high_order_relation_feat(self, data_dict, obj_feats, target_ids):
        rel_feats = data_dict["edge_feature"] # batch_size, num_proposals, num_locals, feat_size
        adjacent_mat = data_dict["adjacent_mat"] # batch_size, num_proposals, num_proposals
        batch_size = rel_feats.shape[0]

        fst_order_rel_feats = torch.gather(rel_feats, 1,
            target_ids.view(batch_size, 1, 1, 1).repeat(1, 1, self.num_locals, self.feat_size)).squeeze(1) # batch_size, num_locals, feat_size
        
        fst_order_nei_indices = torch.gather(adjacent_mat, 1,
            target_ids.view(batch_size, 1, 1).repeat(1, 1, self.num_proposals)).squeeze(1) # batch_size, num_proposals
        fst_order_nei_indices = fst_order_nei_indices.nonzero().view(batch_size, self.num_locals, 2)
        fst_order_nei_indices = fst_order_nei_indices[:, :, 1] # batch_size, num_locals

        fst_order_nei_feats = torch.gather(obj_feats, 1, 
            fst_order_nei_indices.view(batch_size, self.num_locals, 1).repeat(1, 1, self.feat_size)) # batch_size, num_locals, feat_size

        scd_order_nei_indices = torch.zeros(batch_size, self.num_locals, self.num_locals).cuda()
        scd_order_rel_feats = torch.zeros(batch_size, self.num_locals, self.num_locals, self.feat_size).cuda()
        scd_order_nei_feats = torch.zeros(batch_size, self.num_locals, self.num_locals, self.feat_size).cuda()

        for nei_id in range(self.num_locals):
            scd_ord_target_ids = fst_order_nei_indices[:, nei_id]

            scd_ord_rel_feats = torch.gather(rel_feats, 1,
                scd_ord_target_ids.view(batch_size, 1, 1, 1).repeat(1, 1, self.num_locals, self.feat_size)).squeeze(1) # batch_size, num_locals, feat_size
            scd_order_rel_feats[:, nei_id, :, :] = scd_ord_rel_feats

            scd_ord_nei_indices = torch.gather(adjacent_mat, 1,
                scd_ord_target_ids.view(batch_size, 1, 1).repeat(1, 1, self.num_proposals)).squeeze(1) # batch_size, num_proposals
            scd_ord_nei_indices = scd_ord_nei_indices.nonzero().view(batch_size, self.num_locals, 2)
            scd_ord_nei_indices = scd_ord_nei_indices[:, :, 1] # batch_size, num_locals
            scd_order_nei_indices[:, nei_id, :] = scd_ord_nei_indices

            scd_ord_nei_feats = torch.gather(obj_feats, 1, 
                scd_ord_nei_indices.view(batch_size, self.num_locals, 1).repeat(1, 1, self.feat_size))
            scd_order_nei_feats[:, nei_id, :, :] = scd_ord_nei_feats

        return fst_order_rel_feats, fst_order_nei_feats, scd_order_rel_feats, scd_order_nei_feats
    ...
    def _forward_sample_batch(self, data_dict, max_len=CONF.TRAIN.MAX_DES_LEN, min_iou=CONF.TRAIN.MIN_IOU_THRESHOLD):
        ...
        # rel_feats, nei_feats = self._get_relation_feat(data_dict, obj_feats, target_ids)
        fst_order_rel_feats, fst_order_nei_feats, scd_order_rel_feats, scd_order_nei_feats = self._get_high_order_relation_feat(data_dict, obj_feats, target_ids)
        
        target_fst_order_feats_tile = target_feats.unsqueeze(1).repeat(1, fst_order_rel_feats.size(1), 1)
        fst_order_tri_feats = torch.cat([target_fst_order_feats_tile, fst_order_rel_feats, fst_order_nei_feats], -1) # batch_size, num_locals, feat_size*3
        fst_order_node_feats = self.map_fst_order_node_feat(fst_order_tri_feats) # batch_size, num_locals, feat_size

        # high-order representation : B --> C
        # target_scd_order_feats_tile = fst_order_nei_feats.unsqueeze(2).repeat(1, 1, scd_order_rel_feats.size(2), 1) # batch_size, num_locals, num_locals, feat_size
        # high-order representation : A --> B --> C
        target_scd_order_feats_tile = fst_order_node_feats.unsqueeze(2).repeat(1, 1, scd_order_rel_feats.size(2), 1) # batch_size, num_locals, num_locals, feat_size
        scd_order_tri_feats = torch.cat([target_scd_order_feats_tile, scd_order_rel_feats, scd_order_nei_feats], -1) # batch_size, num_locals, num_locals, feat_size*3
        scd_order_node_feats = self.map_scd_order_node_feat(scd_order_tri_feats) # batch_size, num_locals, num_locals, feat_size

        tri_feats = torch.cat([fst_order_node_feats, scd_order_node_feats.view(batch_size, -1, self.feat_size)], dim=1) # batch_size, num_locals+num_locals*num_locals, feat_size
        ...
        step_id = 0
        step_input = word_embs[:, step_id] # batch_size, emb_size
        prev_mask = valid_masks.unsqueeze(-1) * (1./self.num_locals)

        valid_masks = valid_masks.unsqueeze(-1)
        while True:
            # feed
            hidden_1, hidden_2, step_mask, _ = self._step(step_input, target_feats, tri_feats, hidden_1, hidden_2, valid_masks, prev_mask)
            ...
            masks.append(step_mask.detach()) # batch_size, num_proposals, 1
            prev_mask = torch.mean(torch.stack(masks, 0), 0)
    ...
    def _forward_scene_batch(self, data_dict, use_tf=False, max_len=CONF.TRAIN.MAX_DES_LEN):
        ...
        masks = []
        valid_masks = []
        hyper_node_feats = torch.zeros(batch_size, self.num_proposals, self.feat_size).cuda()
        for prop_id in range(self.num_proposals):
            # select object features
            target_feats = obj_feats[:, prop_id] # batch_size, emb_size
            ...
            # rel_feats, nei_feats = self._get_relation_feat(data_dict, prop_obj_feats, target_ids)
            fst_order_rel_feats, fst_order_nei_feats, scd_order_rel_feats, scd_order_nei_feats = self._get_high_order_relation_feat(data_dict, obj_feats, target_ids)
            ...
            prev_mask = valid_prop_masks.unsqueeze(-1) * (1./self.num_locals)
            hyper_node_feat_list = []
            while True:
                # feed
                hidden_1, hidden_2, step_mask, hyper_node_feat = self._step(step_input, target_feats, tri_feats, hidden_1, hidden_2, valid_prop_masks.unsqueeze(-1), prev_mask)
                step_output = self.classifier(hidden_2) # batch_size, num_vocabs

                # save hypernode feat
                hyper_node_feat_list.append(hyper_node_feat)
                ...
                step_output = step_output.unsqueeze(1) # batch_size, 1, num_vocabs
                prop_outputs.append(step_output)
                prop_masks.append(step_mask.detach())

                prev_mask = torch.mean(torch.stack(prop_masks, 0), 0)
            ...
            hyper_node_feat_agg = torch.stack(hyper_node_feat_list, dim=-1)
            hyper_node_feats[:, prop_id, :] = hyper_node_feat_agg.mean(-1)
            prop_outputs = torch.cat(prop_outputs, dim=1).unsqueeze(1) # batch_size, 1, num_words - 1/max_len, num_vocabs
        ...
        return data_dict, hyper_node_feats
