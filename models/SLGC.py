...
class SpatialLayoutConv(MessagePassing):
    def __init__(self, in_size, out_size, heads=1, concat=True, edge_dim=None, bias=True, root_weight=True, aggregation='add',\
        ...
        if edge_dim is not None:
            ...
            self.map_back_edge_loc = Linear(in_size, in_size)
            self.map_back_edge_obj =  nn.Linear(2 * in_size, out_size)
    ...
    def forward(self, x, edge_index, edge_attr):
        out, message = self.propagate(edge_index, x=x, edge_attr=edge_attr)
    ...
    def propagate(self, edge_index: Adj, size: Size = None, **kwargs):
        ...
        msg_kwargs = self.inspector.distribute('message', coll_dict)
        message, edge_raw = self.message(**msg_kwargs)
    ...
    def message(self, x_i, x_j, edge_attr, index, ptr, size_i):
        ...
        if self.lin_edge is not None:
            ...
            edge_loc_feat = mapped_xy_relative_coord
        ...
        # ipdb.set_trace()
        return out, mapped_edge_obj_feat
...
class SLGC(nn.Module):
    def __init__(self, in_size, out_size, num_layers, num_proposals, feat_size, num_locals, glove, 
        ...
        return_orientation=False, num_bins=6, return_distance=False, use_color=True, use_multiview=False, use_cls_embedding=True):
        super().__init__()
        ...
        self.store_edge = True # set False when eval
        ...
        for _ in range(num_layers):
            if graph_mode == "spatial_layout_conv":
                ...
                raise ValueError("invalid graph mode, choices: [\"spatial_layout_conv\", \"edge_conv\"]")
    ...
    def _feed(self, graph):
        feat, edge, edge_attr = graph.x, graph.edge_index, graph.edge_attr

        for layer in self.gc_layers:
            if self.graph_mode == "spatial_layout_conv" or self.graph_mode == "edge_conv":
                feat, message = layer(feat, edge, edge_attr)
    ...
    def forward(self, data_dict):
        ...
        if self.use_cls_embedding:
            ...
            object_centers = data_dict["bbox_center"] # batch_size, num_proposals, 3
        ...
        for batch_id in range(batch_size):
            ...
            obj_sparse_mat = coo_matrix(batch_adjacent_mat.detach().cpu().numpy())
            obj_batch_edge_index, _ = GUtils.from_scipy_sparse_matrix(obj_sparse_mat)
            ...
            if self.use_cls_embedding:
                ...
                batch_node_feats = batch_global_obj_feats

            # ipdb.set_trace()
            batch_graph = GData(x=batch_node_feats, edge_index=batch_edge_index.cuda(), edge_attr=batch_spatial_words_indicator)

            # graph conv
            node_feat, edge_feat = self._feed(batch_graph)

            # get object node features (i.e., exclude global node)
            obj_node_feat = node_feat[:-1, :]
            ...
            if self.store_edge:
                ...
                try:
                    ...
                    edge_feat = edge_feat[:-(num_valid_objects+1),:]

                    #edge_feat = edge_feat.view(num_valid_objects, self.num_locals+1, -1)
                    #edge_feat = edge_feat[:, :-1, :].contiguous().view(num_valid_objects*self.num_locals, -1)
 
                    # print("batch no {}".format(batch_id))
                    if num_valid_objects < self.num_locals:
                        tmp_num_locals = num_valid_objects
                    else:
                        tmp_num_locals = self.num_locals
                    edge_feat = edge_feat.view(num_valid_objects, tmp_num_locals+1, -1)
                    
                    edge_feat = edge_feat[:, :-1, :].contiguous().view(num_valid_objects*tmp_num_locals, -1)

                    num_src_objects = len(set(obj_batch_edge_index[0].cpu().numpy()))
                    ...
                    edge_indices[batch_id, :, :num_src_objects*num_tar_objects] = obj_batch_edge_index[:, :num_src_objects*num_tar_objects]
                    
                    if self.return_orientation:
                        _, edge_feat = self.edge_layer(obj_node_feat, obj_batch_edge_index.cuda())
